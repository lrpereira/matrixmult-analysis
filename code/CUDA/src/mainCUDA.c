...
#define sizeUM_EVEsizeTS 3
#define TIME_RESOLUTIOsize 1000
...
void start(){
	...
	initial_time = (hour * 3600 + min * 60 + sec) * TIME_RESOLUTION;
}
...
void stop(){
	...
	//Convert current time to milliseconds
	int currentTime;
	currentTime = (hour * 3600 + min * 60 + sec) * TIME_RESOLUTION;
...
void run_dotproduct(float *A, float *B, float *C, int SIZE, void (*f) (float*,float*,float*, int)){
	...
	fillMatrices(A, B, C, SIZE);
  	...
  	printf("Execution time taked: %d milliseconds", execTime);
}
